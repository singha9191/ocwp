...
function bosa_fitness_default_styles(){
	...
	$feature_posts_height = get_theme_mod( 'feature_posts_height', 270 );
	$css .= '
	...
	if( $the_custom_logo_url == ''  && ( get_theme_mod( 'header_layout', 'header_two' ) !== 'header_two' || ( !is_front_page() && ( get_theme_mod( 'disable_transparent_header_post', true ) && !is_single() ) && ( get_theme_mod( 'disable_transparent_header_page', true ) && !is_page() ) ) || empty( get_theme_mod( 'header_separate_logo', '' ) ) ) ){
		$css .= '
...
add_action( 'wp_head', 'bosa_fitness_default_styles', 99 );